...
from sqlmesh_ff.utils.paths import get_layer_and_domain, model_path_relative
...
class CustomExclusionsChecker:
    ...
    def __init__(self, context: Context, exclusions_path: Path):
    ...
    def check_model(self, model) -> t.List[str]:
        if model.kind.is_symbolic:
            return []

        violations = []
        model_layer, model_domain = get_layer_and_domain(model._path)
        if not model_layer:
            return []

        for dependency_name in model.depends_on:
            try:
                dependency_model = self.context.get_model(dependency_name)
                if not dependency_model:
                    continue

                dep_layer, dep_domain = get_layer_and_domain(dependency_model._path)
                if not dep_layer:
                    continue
...
def collect_custom_exclusion_findings(
    ...
    checker = CustomExclusionsChecker(context, exclusions_path)
